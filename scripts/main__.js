...
(function (Modernizr, $, GMaps, window, document, navigator) {
  ...
  function setGoogleMaps() {
    if (GMaps) {
      ...
      markers.miraflores = map.addMarker({ // GMarker de Miraflores
        ...
        title: 'Miraflores',
        infoWindow: {
          content: '<div class="scrollFix">' + 'Sede Miraflores<br>447-0162 / 255-9429' + '</div>'
        }
      });
      markers.villa = map.addMarker({ // GMarker de Villa
        ...
        title: 'Villa',
        infoWindow: {
          content: '<div class="scrollFix">' + 'Sede Chorrillos<br>621-2300' + '</div>'
        }
      });
      map.fitZoom();
    }
      ...
      var $nav = $('#nav-dots > span'),
          ...
          initEvents = function () {
            ...
            var container = $('.background-container'),
                eventNames = 'webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend';

            container.on(eventNames, '.background.current', function(e) {
              var current = $(this),
                  next = current.next();
    ...
    if (!supportBlacklist() && !isMobile()) {
      ...
      $.fn.fullpage({
        ...
        'beforePageScroll': function(page) {
          ...
          if (page.attr('id') === 'sec-cursos') {
            page.find('.cf.w-inner').css({ overflow: 'hidden' });
          }
        ...
        'afterSlideLoad': function (anchorLink, index, slideAnchor, slideIndex) {
          ...
          if(anchorLink === 'inicio' && index == 1 && slideIndex == 1){
            // clearInterval(idInterval);
          }
        },
        'onSlideLeave': function(anchorLink, index, slideIndex, direction) {
          if (anchorLink === 'inicio') {
            // setTimeout($.fn.fullpage.moveSlideRight, 3500);
          }
        },
  ...
  $('.see-gallery')
    .bind('click', function(e) {
      ...
      if (isMobile()) {
        $('#sec-nosotros').height(window.innerHeight - 32);
      }
      else {
  ...
  $(document).on('click', '.gallery .item', function(e) {
    var image = $(this).find('img'),
        figure = $(this),
        photoBig = $('#photo-big'),
        photoLegend =$('#photo-legend');

    var imageURL = $(this).find('img').attr('src'),
        imageLegend = $(this).find('img').attr('alt');
